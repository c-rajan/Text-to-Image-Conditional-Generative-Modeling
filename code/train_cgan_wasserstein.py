...
def compute_gradient_penalty(D, condition_embd_expand, real_samples, fake_samples):
...
#             gen_imgs = Variable(model_G.forward(z, condition_embd), requires_grad=True)
            ...
            gradient_penalty = compute_gradient_penalty(model_D, condition_embd_expand, real_imgs.data, gen_imgs.data)
            loss_D = -torch.mean(validity_real) + torch.mean(validity_fake) + 1 * gradient_penalty
            loss_D.backward()
            optimizer_D.step()
