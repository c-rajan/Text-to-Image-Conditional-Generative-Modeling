...
def compute_gradient_penalty(D, condition_embd_expand, real_samples, fake_samples, device):
    ...
    d_interpolates = D(interpolates, condition_embd_expand)
...
def train(model_G, model_D, embedder, optimizer_G, optimizer_D, scheduler_G, scheduler_D,
          train_loader, val_loader, adv_loss, opt, device):
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels, captions) in enumerate(train_loader):
           ...
           # condition_embd = embedder(labels, captions)
           # condition_embd = Variable(condition_embd.unsqueeze(dim=2).unsqueeze(dim=3).to(device))
           # condition_embd_expand = Variable(condition_embd.expand(-1, -1, 32, 32).to(device))

            # ---------------------
            #  Train Discriminator
            # ---------------------
            # 5 times as in WGAN paper
            n_critic = 2
...
#             z = torch.randn(opt.batch_size, opt.z_dim, 1, 1).to(device)
            ...
            z = Tensor(np.random.normal(0, 1, (imgs.shape[0], opt.z_dim, 1, 1))).to(device)
            z = Variable(z)
...
#             gen_imgs = Variable(model_G.forward(z, condition_embd), requires_grad=True)
            ...
            gradient_penalty = compute_gradient_penalty(model_D, condition_embd_expand, real_imgs.data, gen_imgs.data, device)
            loss_D = -torch.mean(validity_real) + torch.mean(validity_fake) + 10 * gradient_penalty
            loss_D.backward()
            optimizer_D.step()
...
if __name__ == "__main__":
    ...
    device = torch.device("cuda:0") if (torch.cuda.is_available() and opt.use_cuda) else torch.device("cpu")
    ...
    b1 = 0.5
    b2 = 0.9
    optimizer_G = torch.optim.Adam(model_G.parameters(), lr=opt.lr, betas=(b1, b2))
    optimizer_D = torch.optim.Adam(model_D.parameters(), lr=opt.lr, betas=(b1, b2))
    ...
    if opt.train:
        train(model_G=model_G, model_D=model_D, embedder=encoder, optimizer_G=optimizer_G, optimizer_D=optimizer_D,
              scheduler_G=scheduler_G, scheduler_D=scheduler_D, train_loader=train_dataloader, val_loader=val_dataloader, adv_loss=adversarial_loss, opt=opt, device=device)
    else:
